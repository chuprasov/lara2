                ...
                <form action="{{ route('catalog', $category) }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input name="filters[price][from]" value="{{ request('filters.price.from', 0) }}" type="number"
                            ...
                            <input name="filters[price][to]" value="{{ request('filters.price.to', 100000) }}"
                                type="number"
                                class="w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs shadow-transparent outline-0 transition"
                    ...
                    <div>
                        ...
                        @foreach ($brands as $brand)
                            <div class="form-checkbox">
                                <input name="filters[brands][{{ $brand->id }}]" type="checkbox"
                                    value="{{ $brand->id }}" @checked(request('filters.brands.' . $brand->id))
